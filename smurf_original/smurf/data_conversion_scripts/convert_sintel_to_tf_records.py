  ...
  for data_split in ['training', 'test']:
    ...
    for data_type in ['clean', 'final']:
      ...
      if data_split == 'training':
        ...
        segment_folder = os.path.join(FLAGS.data_dir, data_split,
                                      'segmentation')
        segments_invalid_folder = os.path.join(FLAGS.data_dir,
                                               data_split,
                                               'segmentation_invalid')
      else:
        segment_folder = None
      ...
      if FLAGS.shard is None:  # Write all shards in this case.
...
def write_records(data_list, output_folder, shard):
  ...
  with tf.io.TFRecordWriter(filenames[shard]) as record_writer:
    ...
    for i, (images, flow, occlusion, invalids, segments,
      ...
      def imread(x):
          return np.asarray(Image.open(x))
      image1_data = imread(images[0])
      image2_data = imread(images[1])
      if flow is not None:
        ...
        occlusion_data = np.expand_dims(
            imread(occlusion) // 255, axis=-1)
        invalid1_data = np.expand_dims(
            imread(invalids[0]) // 255, axis=-1)
        invalid2_data = np.expand_dims(
            imread(invalids[1]) // 255, axis=-1)
        segment1_data = np.expand_dims(
            imread(segments[0]), axis=-1)
        segment2_data = np.expand_dims(
            imread(segments[1]), axis=-1)
        segment_invalid1_data = np.expand_dims(
            imread(segments_invalid[0]), axis=-1)
        segment_invalid2_data = np.expand_dims(
            imread(segments_invalid[1]), axis=-1)
