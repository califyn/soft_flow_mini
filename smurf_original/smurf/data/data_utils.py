...
def parse_data(proto,
  ...
  def deserialize(s, dtype, dims):
    ret = tf.io.decode_raw(s, dtype) 
    #tf.print(context_parsed)
    #tf.print(tf.shape(ret))
    if dims == 3 or dims == 1:
        #tf.print(tf.math.reduce_max(ret))
        #tf.print(tf.math.reduce_min(ret))
        #return tf.image.convert_image_dtype(tf.reshape(ret, [context_parsed['height'], context_parsed['width'], dims]), tf.uint8)
        return tf.reshape(ret, [context_parsed['height'], context_parsed['width'], dims])
    else:
        return tf.reshape(ret, [context_parsed['height'], context_parsed['width'], dims])
    """
    return tf.reshape(
  ...
  images = tf.map_fn(
      ...
      #lambda s: deserialize(s, tf.float32, 3),
  ...
  if include_occlusion:
    ...
    #occlusion_mask = deserialize(context_parsed['occlusion_mask'], tf.float32, 1)
    if height is not None and width is not None:
  ...
  if include_invalid:
    invalid_masks = tf.map_fn(
        ...
        #lambda s: deserialize(s, tf.float32, 1),
