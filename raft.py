...
class UpdateBlock(nn.Module):
    ...
    def forward(self, hidden_state, context, corr_features, pyr_features, flow):
        ...
        delta_corr = self.flow_head(hidden_state)
        #new_corr = corr_features + delta_corr
        #return hidden_state, delta_flow
        new_corr = corr_features

        new_corr = torch.nn.functional.softmax(new_corr, dim=1)[..., None] # (B, C, H, W, X)
        new_features = torch.permute(torch.sum(new_corr * pyr_features, dim=1), (0, 3, 1, 2))

        return hidden_state, new_features
...
class CorrBlock(nn.Module):
    ...
    def build_pyramid(self, fmap1, fmap2):
        ...
        feat_volume = fmap2
        self.feat_pyramid = [feat_volume]
        for _ in range(self.num_levels - 1):
            feat_volume = F.avg_pool2d(feat_volume, kernel_size=2, stride=2)
            self.feat_pyramid.append(feat_volume)
    ...
    def index_pyramid(self, centroids_coords):
        ...
        indexed_pyramid = []
        indexed_feat_pyramid = []
        for corr_volume, feat_volume in zip(self.corr_pyramid, self.feat_pyramid):
            ...
            # hopefully the memory usage here is ok
            sampling_coords = sampling_coords.reshape((batch_size, h*w, sampling_coords.shape[-2]**2, 2))
            indexed_feat_volume = grid_sample(feat_volume, sampling_coords, align_corners=True, mode="bilinear").view(
                batch_size, -1, h, w, sampling_coords.shape[-2], 
            )

            indexed_pyramid.append(indexed_corr_volume)
            indexed_feat_pyramid.append(indexed_feat_volume)
            centroids_coords = centroids_coords / 2

        corr_features = torch.cat(indexed_pyramid, dim=-1).permute(0, 3, 1, 2).contiguous()
        pyr_features = torch.cat(indexed_feat_pyramid, dim=-1).permute(0, 4, 2, 3, 1).contiguous()
        ...
        return corr_features, pyr_features
...
class RAFT(nn.Module):
    def __init__(self, *, feature_encoder, context_encoder, corr_block, update_block, mask_predictor=None, out_feat_dim=None):
        """RAFT model from
        ...
        self.corr_blocks = [corr_block, deepcopy(corr_block)]
        ...
        self.weight_sl = None # modify this later
        self.temp = torch.nn.Parameter(torch.Tensor([0.01] * 24))

        if out_feat_dim is not None:
            self.reshape_feats = nn.Conv2d(self.feature_encoder.output_dim, out_feat_dim, kernel_size=1)

        """
        self.merge_features = nn.Sequential(
            torch.nn.Conv2d(self.feature_encoder.output_dim * 2, self.feature_encoder.output_dim, kernel_size=1),
            torch.nn.ReLU(),
            torch.nn.Conv2d(self.feature_encoder.output_dim, self.feature_encoder.output_dim, kernel_size=1),
            torch.nn.ReLU(),
            torch.nn.Conv2d(self.feature_encoder.output_dim, self.feature_encoder.output_dim, kernel_size=1)
        )
        """
    ...
    def forward(self, image1, image2, num_flow_updates: int = 12):
        ...
        self.corr_blocks[0].build_pyramid(fmap1, fmap2)
        self.corr_blocks[1].build_pyramid(fmap2, fmap1)

        context_out_1 = self.context_encoder(image1)
        context_out_2 = self.context_encoder(image2)
        if context_out_1.shape[-2:] != (h // 8, w // 8):
        ...
        out_channels_context = context_out_1.shape[1] - hidden_state_size
        ...
        hidden_state_1, context_1 = torch.split(context_out_1, [hidden_state_size, out_channels_context], dim=1)
        hidden_state_1 = torch.tanh(hidden_state_1)
        context_1 = F.relu(context_1)
        hidden_state_2, context_2 = torch.split(context_out_2, [hidden_state_size, out_channels_context], dim=1)
        hidden_state_2 = torch.tanh(hidden_state_2)
        context_2 = F.relu(context_2)

        if num_flow_updates == 0:
            up_mask_1 = None if self.mask_predictor is None else self.mask_predictor(hidden_state_1)
            up_mask_2 = None if self.mask_predictor is None else self.mask_predictor(hidden_state_2)
            upsampled_feats_1 = upsample_flow(flow=self.reshape_feats(fmap1), up_mask=up_mask_1)
            upsampled_feats_2 = upsample_flow(flow=self.reshape_feats(fmap2), up_mask=up_mask_2)
            return upsampled_feats_1, upsampled_feats_2

        #coords0 = make_coords_grid(batch_size, h // 8, w // 8).to(fmap1.device)
        coords1 = make_coords_grid(batch_size, h // 8, w // 8).to(fmap1.device)
        coords2 = make_coords_grid(batch_size, h // 8, w // 8).to(fmap1.device)

        #flow_predictions = []
        new_features_1 = fmap1
        new_features_2 = fmap2
        for i in range(num_flow_updates):
            coords1 = coords1.detach()  # Don't backpropagate gradients through this branch, see paper
            corr_features_1, pyr_features_1 = self.corr_blocks[0].index_pyramid(centroids_coords=coords1)

            coords2 = coords2.detach()  # Don't backpropagate gradients through this branch, see paper
            corr_features_2, pyr_features_2 = self.corr_blocks[1].index_pyramid(centroids_coords=coords2)

            #flow = coords1 - coords0
            #hidden_state, delta_flow = self.update_block(hidden_state, context, corr_features, flow)
            hidden_state_1, out_features_1 = self.update_block(hidden_state_1, context_1, corr_features_1.detach(), pyr_features_1.detach(), coords1)
            hidden_state_2, out_features_2 = self.update_block(hidden_state_2, context_2, corr_features_2.detach(), pyr_features_2.detach(), coords2)

            new_features_1 = self.merge_features(torch.cat((new_features_1, out_features_1), dim=1))
            new_features_2 = self.merge_features(torch.cat((new_features_2, out_features_2), dim=1))

            if i == num_flow_updates - 1:
                up_mask_1 = None if self.mask_predictor is None else self.mask_predictor(hidden_state_1)
                up_mask_2 = None if self.mask_predictor is None else self.mask_predictor(hidden_state_2)
                #upsampled_flow = upsample_flow(flow=(coords1 - coords0), up_mask=up_mask)
                upsampled_feats_1 = upsample_flow(flow=self.reshape_feats(new_features_1), up_mask=up_mask_1)
                upsampled_feats_2 = upsample_flow(flow=self.reshape_feats(new_features_2), up_mask=up_mask_2)

                return upsampled_feats_1, upsampled_feats_2
                #flow_predictions.append(upsampled_flow)
            else:
                #coords1 = coords1 + delta_flow
                temp = self.temp[i].to(new_features_1.device)
                coords1 = flow_from_feats(new_features_2, new_features_1, self.weight_sl, temp)
                coords2 = flow_from_feats(new_features_1, new_features_2, self.weight_sl, temp)

                self.corr_blocks[0].build_pyramid(new_features_1, new_features_2)
                self.corr_blocks[1].build_pyramid(new_features_2, new_features_1)
...
def _raft(
    ...
    **kwargs,
    ...
    if update_block is None:
        ...
        flow_head = FlowHead(in_channels=recurrent_block_hidden_state_size, hidden_size=flow_head_hidden_size, out_channels=corr_block.out_channels)
...
def my_raft_large(*, weights: Optional[Raft_Large_Weights] = None, progress=True, out_feat_dim=None, **kwargs) -> RAFT:
    """RAFT model from
    `RAFT: Recurrent All Pairs Field Transforms for Optical Flow <https://arxiv.org/abs/2003.12039>`_.
