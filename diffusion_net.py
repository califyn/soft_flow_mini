...
class DiffusionNetBlock(nn.Module):
    ...
    def __init__(self, C_width, mlp_hidden_dims, image_shape,
                 dropout=True, 
    ...
    def forward(self, x_in, mass, L, evals, evecs, gradX, gradY):
        ...
        if self.with_gradient_features:
            ...
            for b in range(B):
                ...
                x_diffuse_unpacked = torch.reshape(x_diffuse[b, ...], (*self.image_shape, x_diffuse.shape[-1]))
                x_gradX, x_gradY = torch.gradient(x_diffuse_unpacked, dim=[0, 1])
                x_gradX = torch.reshape(x_gradX, (x_diffuse.shape[1], x_gradX.shape[-1]))
                x_gradY = torch.reshape(x_gradY, (x_diffuse.shape[1], x_gradY.shape[-1]))
...
class DiffusionNet(nn.Module):
    ...
    def __init__(self, C_in, C_out, image_shape, C_width=128, N_block=4, last_activation=None, outputs_at='vertices', mlp_hidden_dims=None, dropout=True, 
